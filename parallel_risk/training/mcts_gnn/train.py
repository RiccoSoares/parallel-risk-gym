...
Usage:
    python -m parallel_risk.training.mcts_gnn.train \
        --config parallel_risk/training/mcts_gnn/configs/mcts_gnn_baseline.yaml \
        --num-iterations 100
"""
...
def _build_trainer_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """Flatten the YAML into the trainer's expected schema."""
    return {
        'env': cfg['env'],
        'model': cfg['model'],
        'trainer': cfg['trainer'],
        'log_dir': cfg.get('logging', {}).get('log_dir', 'runs/mcts_gnn_training'),
        'checkpoint_dir': cfg.get('logging', {}).get('checkpoint_dir', 'checkpoints/mcts_gnn_training'),
    }
...
def run(cfg: Dict[str, Any], num_iterations: int, resume: str = None) -> None:
    trainer = MCTSGNNTrainer(_build_trainer_config(cfg))
    start_iteration = 0
    if resume:
        start_iteration = trainer.load_checkpoint(resume)
    ...
    for iteration in range(start_iteration, start_iteration + num_iterations):
        ...
        # Episode outcome summary (one z per (agent, turn); average is fine
        # as a rough training signal).
        ...
        if (iteration + 1) % ckpt_interval == 0:
            path = ckpt_dir / f"mcts_gnn_iter_{iteration+1:06d}.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MCTS+GNN distillation training")
    parser.add_argument("--config", type=str, required=True, help="YAML config path")
    parser.add_argument("--num-iterations", type=int, default=None,
                        help="Override outer.num_iterations from the YAML")
    parser.add_argument("--resume", type=str, default=None,
                        help="Resume from a PPO or MCTS+GNN checkpoint")
    ...
    if num_iterations is None:
        num_iterations = int(cfg.get('outer', {}).get('num_iterations', 100))
