"""Real MCTS+GNN training run with periodic evaluation and dashboard plots.

Runs an actual training loop long enough to see whether the model is
learning, evaluates it periodically against MCTS(uniform) at the same
search budget, and writes a 3-panel dashboard summarising the run.

Usage (defaults tuned for a ~30-45 minute run on a modern desktop):
    PYTHONPATH=. python experiments/mcts_gnn_training_run.py

Key knobs:
    --num-iterations         total outer-loop iterations
    --num-games-per-iter     self-play games per iteration
    --num-workers            spawn workers for self-play (CPU)
    --mcts-budget            MCTS simulations per move (self-play + eval)
    --eval-interval          run eval every N iterations
    --num-eval-games         games per eval event
    --map-name               map to train on (single map)
    --use-gpu / --no-gpu     put trainer update on GPU (default: on)
"""

from __future__ import annotations

# Load torch first: importing parallel_risk.training triggers this package's
# __init__ which pulls RLlib, whose try_import_torch path can fail on Windows
# if torch isn't already in sys.modules. Same footgun as the smoke script.
import torch  # noqa: F401

import argparse
import json
import math
import multiprocessing as mp
import time
from concurrent.futures import ProcessPoolExecutor
from datetime import datetime
from pathlib import Path
from typing import Any, Dict, List

import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import numpy as np

from parallel_risk.training.mcts_gnn.self_play import collect_games
from parallel_risk.training.mcts_gnn.trainer import MCTSGNNTrainer


# ---------------------------------------------------------------------------
# Parallel eval worker (module-level so ProcessPoolExecutor can pickle it)
# ---------------------------------------------------------------------------

def _eval_worker(args):
    """Play `num_games` games of MCTSGNNAgent vs MCTSAgent(uniform).

    Alternates colors — half the games the GNN plays agent_0, half agent_1 —
    so eval results are symmetric under any starting-position advantage.
    Runs on CPU in a spawned subprocess.
    """
    (state_dict_cpu, model_kwargs, map_name, max_turns, action_budget,
     mcts_budget, num_games, base_seed, max_regions) = args

    import numpy as np
    import torch
    from parallel_risk import ParallelRiskEnv
    from parallel_risk.agents.mcts_agent import MCTSAgent
    from parallel_risk.agents.mcts_gnn_agent import MCTSGNNAgent
    from parallel_risk.models.action_decoder import ActionDecoder
    from parallel_risk.models.gnn_gcn import GCNPolicy

    torch.set_num_threads(1)

    policy = GCNPolicy(**model_kwargs)
    policy.load_state_dict(state_dict_cpu)
    policy.eval()

    env = ParallelRiskEnv(map_name=map_name, max_turns=max_turns,
                          reward_shaping_config=None)
    decoder = ActionDecoder(action_budget=action_budget, max_troops=20)
    gnn_agent = MCTSGNNAgent(
        policy=policy, decoder=decoder, map_config=env.map_config,
        simulation_budget=mcts_budget, c_puct=1.4, action_budget=action_budget,
        max_turns=max_turns, device='cpu', max_regions=max_regions,
    )
    uniform_agent = MCTSAgent.from_env(env, simulation_budget=mcts_budget,
                                       action_budget=action_budget)

    wins = losses = draws = 0
    for g in range(num_games):
        # Alternate colors each game so first-mover advantage cancels out.
        gnn_plays_agent_0 = (g % 2 == 0)
        obs, _ = env.reset(seed=base_seed + g)
        done = False
        while not done:
            actions = {}
            for aid in ('agent_0', 'agent_1'):
                if aid not in obs:
                    continue
                pick_gnn = (aid == 'agent_0') == gnn_plays_agent_0
                if pick_gnn:
                    actions[aid] = gnn_agent.get_action(env.game_state, aid)
                else:
                    actions[aid] = uniform_agent.get_action(env.game_state, aid)
            obs, rewards, terminateds, truncateds, _ = env.step(actions)
            done = terminateds.get('__all__', False) or truncateds.get('__all__', False)

        gnn_aid = 'agent_0' if gnn_plays_agent_0 else 'agent_1'
        opp_aid = 'agent_1' if gnn_plays_agent_0 else 'agent_0'
        r_gnn = float(rewards.get(gnn_aid, 0.0))
        r_opp = float(rewards.get(opp_aid, 0.0))
        if r_gnn > r_opp:
            wins += 1
        elif r_gnn < r_opp:
            losses += 1
        else:
            draws += 1

    return {'wins': wins, 'losses': losses, 'draws': draws,
            'total_games': wins + losses + draws}


def evaluate_vs_uniform(policy, model_kwargs, map_name, max_turns,
                        action_budget, mcts_budget, num_games, num_workers,
                        max_regions, base_seed) -> Dict[str, float]:
    """Evaluate the current GNN via MCTS+GNN vs MCTS(uniform) at same budget."""
    state_dict_cpu = {k: v.detach().cpu() for k, v in policy.state_dict().items()}
    per_worker = max(1, num_games // max(1, num_workers))
    worker_args = [
        (state_dict_cpu, model_kwargs, map_name, max_turns, action_budget,
         mcts_budget, per_worker, base_seed + w * 1000, max_regions)
        for w in range(max(1, num_workers))
    ]
    ctx = mp.get_context('spawn')
    with ProcessPoolExecutor(max_workers=num_workers, mp_context=ctx) as ex:
        results = list(ex.map(_eval_worker, worker_args))

    wins = sum(r['wins'] for r in results)
    losses = sum(r['losses'] for r in results)
    draws = sum(r['draws'] for r in results)
    total = wins + losses + draws
    return {
        'wins': wins, 'losses': losses, 'draws': draws, 'total': total,
        'win_rate': wins / total if total else 0.0,
        'draw_rate': draws / total if total else 0.0,
    }


# ---------------------------------------------------------------------------
# Plotting
# ---------------------------------------------------------------------------

_SURFACE = '#fcfcfb'
_INK_PRIMARY = '#0b0b0b'
_INK_SECONDARY = '#52514e'
_INK_MUTED = '#898781'
_GRIDLINE = '#e1e0d9'


def _chrome(ax):
    ax.set_facecolor(_SURFACE)
    ax.yaxis.grid(True, color=_GRIDLINE, linewidth=0.8, zorder=0)
    ax.set_axisbelow(True)
    for spine in ('top', 'right'):
        ax.spines[spine].set_visible(False)
    for spine in ('left', 'bottom'):
        ax.spines[spine].set_color(_GRIDLINE)
    ax.tick_params(colors=_INK_MUTED, labelsize=10)


def plot_dashboard(metrics: Dict[str, list], eval_history: List[Dict[str, float]],
                   output_path: Path, map_name: str) -> None:
    """3-panel stacked dashboard: losses, eval win-rate, self-play stats."""
    cmap = plt.get_cmap('tab10')
    C_POLICY = cmap(0)
    C_VALUE = cmap(1)
    C_TOTAL = cmap(2)
    C_EVAL = cmap(3)
    C_WIN = cmap(4)
    C_DRAW = cmap(5)
    C_LEN = cmap(6)

    iters = metrics['iteration']

    fig, axes = plt.subplots(3, 1, figsize=(10, 12), facecolor=_SURFACE)

    # ---- Panel A: losses ----------------------------------------------
    ax = axes[0]
    ax.plot(iters, metrics['policy_loss'], color=C_POLICY, lw=2, label='policy')
    ax.plot(iters, metrics['value_loss'], color=C_VALUE, lw=2, label='value')
    ax.plot(iters, metrics['total_loss'], color=C_TOTAL, lw=2, linestyle='--',
            label='total')
    ax.set_ylabel('loss', color=_INK_SECONDARY, fontsize=11)
    ax.set_title(f'MCTS+GNN training on {map_name} — losses',
                 color=_INK_PRIMARY, fontsize=13, fontweight='bold', pad=6)
    ax.legend(frameon=True, facecolor=_SURFACE, edgecolor=_GRIDLINE,
              labelcolor=_INK_SECONDARY, fontsize=10, loc='upper right')
    _chrome(ax)

    # ---- Panel B: eval win-rate vs MCTS(uniform) ---------------------
    ax = axes[1]
    if eval_history:
        eval_iters = [e['iteration'] for e in eval_history]
        win_rates = [e['win_rate'] * 100 for e in eval_history]
        draw_rates = [e['draw_rate'] * 100 for e in eval_history]
        ax.plot(eval_iters, win_rates, color=C_EVAL, lw=2, marker='o',
                markersize=8, markerfacecolor=_SURFACE, markeredgewidth=2,
                markeredgecolor=C_EVAL, label='win rate')
        ax.plot(eval_iters, draw_rates, color=_INK_MUTED, lw=1, marker='s',
                markersize=5, alpha=0.6, label='draw rate')
    ax.axhline(50, color=_INK_MUTED, lw=1, linestyle='--', alpha=0.6, zorder=1)
    ax.text(min(iters) if iters else 0, 51.5, '50 %',
            color=_INK_MUTED, fontsize=9, va='bottom')
    ax.set_ylim(0, 100)
    ax.set_ylabel('% vs MCTS(uniform)', color=_INK_SECONDARY, fontsize=11)
    ax.set_title('Eval: MCTS+GNN vs MCTS(uniform) — same simulation budget',
                 color=_INK_PRIMARY, fontsize=13, fontweight='bold', pad=6)
    ax.legend(frameon=True, facecolor=_SURFACE, edgecolor=_GRIDLINE,
              labelcolor=_INK_SECONDARY, fontsize=10, loc='lower right')
    _chrome(ax)

    # ---- Panel C: self-play sanity metrics ----------------------------
    ax = axes[2]
    ax.plot(iters, [w * 100 for w in metrics['selfplay_win_frac']],
            color=C_WIN, lw=1.5, label='agent_0 win %', alpha=0.85)
    ax.plot(iters, [d * 100 for d in metrics['selfplay_draw_frac']],
            color=C_DRAW, lw=1.5, label='draw %', alpha=0.85)
    ax.set_ylabel('%', color=_INK_SECONDARY, fontsize=11)
    ax.set_ylim(0, 100)
    ax.set_xlabel('iteration', color=_INK_SECONDARY, fontsize=11)
    ax.legend(frameon=True, facecolor=_SURFACE, edgecolor=_GRIDLINE,
              labelcolor=_INK_SECONDARY, fontsize=10, loc='upper left')
    _chrome(ax)
    # Secondary axis for avg game length
    ax2 = ax.twinx()
    ax2.plot(iters, metrics['avg_game_length'], color=C_LEN, lw=1.5,
             linestyle=':', label='avg game length')
    ax2.set_ylabel('avg turns / game', color=C_LEN, fontsize=11)
    ax2.tick_params(colors=_INK_MUTED, labelsize=10)
    ax2.spines['top'].set_visible(False)
    ax2.legend(frameon=True, facecolor=_SURFACE, edgecolor=_GRIDLINE,
               labelcolor=_INK_SECONDARY, fontsize=10, loc='upper right')
    ax.set_title('Self-play sanity: outcome mix + game length',
                 color=_INK_PRIMARY, fontsize=13, fontweight='bold', pad=6)

    fig.suptitle(f'MCTS+GNN training run — {map_name}',
                 color=_INK_PRIMARY, fontsize=14, fontweight='bold')
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    fig.savefig(output_path, dpi=150, facecolor=_SURFACE)
    plt.close(fig)


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def _selfplay_game_stats(examples: List[Dict[str, Any]]) -> Dict[str, float]:
    """Reduce a batch of examples to per-game outcome and length stats."""
    # Each turn contributes 2 examples (one per agent). Group by (map, agent_0's z)
    # to identify games — cheaper: bucket by their sequential appearance and the
    # trailing z's per agent_0. Simplest robust approach: count all agent_0
    # examples and infer outcomes from z.
    agent_0_zs = [ex['z'] for ex in examples if ex['agent_id'] == 'agent_0']
    # Each turn adds one agent_0 example, so agent_0_zs length ~ total turns.
    # For per-game outcome we need to know when a game ends; but z is
    # constant within a game (all turns in a game share the same terminal
    # outcome), so consecutive equal z's belong to the same game.
    outcomes = []
    turns_per_game = []
    if agent_0_zs:
        cur = agent_0_zs[0]
        cur_len = 1
        for z in agent_0_zs[1:]:
            if z == cur:
                cur_len += 1
            else:
                outcomes.append(cur)
                turns_per_game.append(cur_len)
                cur = z
                cur_len = 1
        outcomes.append(cur)
        turns_per_game.append(cur_len)

    outcomes_arr = np.array(outcomes)
    total = len(outcomes_arr)
    if total == 0:
        return {'win_frac': 0.0, 'draw_frac': 0.0, 'avg_game_length': 0.0}
    return {
        'win_frac': float(np.mean(outcomes_arr > 0)),
        'draw_frac': float(np.mean(outcomes_arr == 0)),
        'avg_game_length': float(np.mean(turns_per_game)),
    }


def main():
    parser = argparse.ArgumentParser(
        description="MCTS+GNN training run with eval + dashboard plots")
    parser.add_argument('--output-dir', default='experiments/mcts_gnn_training_run')
    parser.add_argument('--map-name', default='simple_6',
                        help='Single map to train on (multi-map is supported by '
                             'the trainer but this script trains one map for a '
                             'cleaner learning signal).')
    parser.add_argument('--num-iterations', type=int, default=30)
    parser.add_argument('--num-games-per-iter', type=int, default=32)
    parser.add_argument('--num-workers', type=int, default=8)
    parser.add_argument('--num-epochs', type=int, default=4)
    parser.add_argument('--mcts-budget', type=int, default=50)
    parser.add_argument('--max-turns', type=int, default=30)
    parser.add_argument('--eval-interval', type=int, default=5)
    parser.add_argument('--num-eval-games', type=int, default=16)
    parser.add_argument('--eval-workers', type=int, default=8)
    parser.add_argument('--use-gpu', dest='use_gpu', action='store_true', default=True)
    parser.add_argument('--no-gpu', dest='use_gpu', action='store_false')
    parser.add_argument('--hidden-dim', type=int, default=128)
    parser.add_argument('--num-layers', type=int, default=3)
    parser.add_argument('--checkpoint-interval', type=int, default=10)
    parser.add_argument('--resume', type=str, default=None,
                        help='Path to a PPO or MCTS+GNN checkpoint to warm-start from.')
    parser.add_argument('--reset-optimizer', dest='reset_optimizer',
                        action='store_true', default=True,
                        help='Discard the resumed checkpoint\'s optimizer state (default).')
    parser.add_argument('--keep-optimizer', dest='reset_optimizer',
                        action='store_false',
                        help='Keep the resumed checkpoint\'s optimizer state (Adam moments).')
    args = parser.parse_args()

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = output_dir / 'checkpoints'
    ckpt_dir.mkdir(exist_ok=True)

    print("=" * 70)
    print(f"MCTS+GNN training run  |  map={args.map_name}")
    print("=" * 70)
    print(f"  iterations={args.num_iterations}  games/iter={args.num_games_per_iter}  "
          f"workers={args.num_workers}  epochs/iter={args.num_epochs}")
    print(f"  mcts_budget={args.mcts_budget}  max_turns={args.max_turns}  "
          f"eval every {args.eval_interval} iters "
          f"({args.num_eval_games} games, {args.eval_workers} workers)")
    print(f"  model: hidden={args.hidden_dim} layers={args.num_layers}  "
          f"trainer device={'GPU' if args.use_gpu else 'CPU'}")
    print(f"  output: {output_dir}")

    trainer_cfg = {
        'env': {'map_names': [args.map_name], 'max_turns': args.max_turns,
                'action_budget': 5},
        'model': {'type': 'gcn', 'hidden_dim': args.hidden_dim,
                  'num_layers': args.num_layers, 'dropout': 0.1},
        'trainer': {
            'learning_rate': 1e-4,
            # value_loss_coeff=5 balances value MSE (~0.5) against the
            # joint-action cross-entropy policy loss (~18 with a huge
            # action space). Without this bump the value head starves —
            # policy gradients dominate the shared backbone by ~40:1.
            'value_loss_coeff': 5.0,
            'entropy_coeff': 0.0,
            'max_grad_norm': 0.5,
            'use_gpu': args.use_gpu,
        },
        'log_dir': str(output_dir / 'runs'),
        'checkpoint_dir': str(ckpt_dir),
    }
    trainer = MCTSGNNTrainer(trainer_cfg)
    print(f"  trainer initialized on {trainer.device}  "
          f"(max_regions={trainer.max_regions}, "
          f"node_dim={trainer.node_features_dim})")

    if args.resume:
        loaded_iter = trainer.load_checkpoint(
            args.resume, load_optimizer=(not args.reset_optimizer),
        )
        print(f"  RESUMED from {args.resume} (iter {loaded_iter})  "
              f"optimizer_reset={args.reset_optimizer}")

    env_configs = [{'map_name': args.map_name, 'max_turns': args.max_turns}]
    mcts_config = {
        'simulation_budget': args.mcts_budget,
        'c_puct': 1.4, 'uct_c': 1.41, 'pw_alpha': 0.5,
        'max_rollout_turns': args.max_turns,
        'action_budget': 5,
    }
    self_play_config = {
        'dirichlet_alpha': 0.3, 'noise_frac': 0.25,
        'temperature_turns': 8, 'max_temperature': 1.0,
    }

    metrics = {
        'iteration': [], 'policy_loss': [], 'value_loss': [], 'total_loss': [],
        'selfplay_win_frac': [], 'selfplay_draw_frac': [], 'avg_game_length': [],
        'collect_s': [], 'train_s': [],
    }
    eval_history: List[Dict[str, float]] = []

    run_start = time.perf_counter()
    for it in range(args.num_iterations):
        t0 = time.perf_counter()
        examples = collect_games(
            policy=trainer.policy,
            model_kwargs=trainer.model_kwargs(),
            env_configs=env_configs,
            mcts_config=mcts_config,
            self_play_config=self_play_config,
            num_games=args.num_games_per_iter,
            num_workers=args.num_workers,
            base_seed=it * 10000,
            max_regions=trainer.max_regions,
        )
        collect_s = time.perf_counter() - t0

        t1 = time.perf_counter()
        last: Dict[str, float] = {}
        for _ in range(args.num_epochs):
            last = trainer.update(examples)
        train_s = time.perf_counter() - t1

        sp_stats = _selfplay_game_stats(examples)
        metrics['iteration'].append(it + 1)
        metrics['policy_loss'].append(float(last['policy_loss']))
        metrics['value_loss'].append(float(last['value_loss']))
        metrics['total_loss'].append(float(last['total_loss']))
        metrics['selfplay_win_frac'].append(sp_stats['win_frac'])
        metrics['selfplay_draw_frac'].append(sp_stats['draw_frac'])
        metrics['avg_game_length'].append(sp_stats['avg_game_length'])
        metrics['collect_s'].append(collect_s)
        metrics['train_s'].append(train_s)

        for k in ('total_loss', 'policy_loss', 'value_loss'):
            assert math.isfinite(metrics[k][-1]), \
                f"iter {it}: non-finite {k}={metrics[k][-1]}"

        print(
            f"  iter {it+1:3d}/{args.num_iterations}  "
            f"examples={len(examples):4d}  "
            f"loss={last['total_loss']:6.3f} "
            f"(p={last['policy_loss']:5.3f} v={last['value_loss']:5.3f})  "
            f"win={sp_stats['win_frac']:.2f} draw={sp_stats['draw_frac']:.2f} "
            f"len={sp_stats['avg_game_length']:.1f}  "
            f"collect={collect_s:4.1f}s train={train_s:4.1f}s"
        )

        # Periodic eval
        if (it + 1) % args.eval_interval == 0:
            t2 = time.perf_counter()
            eval_res = evaluate_vs_uniform(
                policy=trainer.policy,
                model_kwargs=trainer.model_kwargs(),
                map_name=args.map_name,
                max_turns=args.max_turns,
                action_budget=5,
                mcts_budget=args.mcts_budget,
                num_games=args.num_eval_games,
                num_workers=args.eval_workers,
                max_regions=trainer.max_regions,
                base_seed=100000 + (it + 1) * 137,
            )
            eval_s = time.perf_counter() - t2
            eval_res['iteration'] = it + 1
            eval_res['eval_time_s'] = eval_s
            eval_history.append(eval_res)
            print(
                f"    [eval @ iter {it+1}]  "
                f"win_rate={eval_res['win_rate']:.2%} "
                f"({eval_res['wins']}W/{eval_res['losses']}L/{eval_res['draws']}D)  "
                f"eval_time={eval_s:.1f}s"
            )

        if (it + 1) % args.checkpoint_interval == 0:
            path = ckpt_dir / f"mcts_gnn_iter_{it+1:06d}.pt"
            trainer.save_checkpoint(path, it + 1)

    run_s = time.perf_counter() - run_start
    trainer.save_checkpoint(ckpt_dir / f"mcts_gnn_iter_{args.num_iterations:06d}.pt",
                            args.num_iterations)
    trainer.close()

    # Dump results
    results = {
        'timestamp': datetime.now().isoformat(),
        'wall_clock_s': run_s,
        'args': vars(args),
        'metrics': metrics,
        'eval_history': eval_history,
    }
    results_path = output_dir / 'results.json'
    with open(results_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved metrics -> {results_path}")

    dashboard_path = output_dir / 'dashboard.png'
    plot_dashboard(metrics, eval_history, dashboard_path, args.map_name)
    print(f"Saved dashboard -> {dashboard_path}")

    print(f"\nTotal wall-clock: {run_s/60:.1f} min")
    if eval_history:
        first_wr = eval_history[0]['win_rate']
        last_wr = eval_history[-1]['win_rate']
        print(f"Eval win-rate: first={first_wr:.2%}  last={last_wr:.2%}  "
              f"delta={(last_wr - first_wr):+.2%}")


if __name__ == "__main__":
    main()
